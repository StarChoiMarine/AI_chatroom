...
- 로그인되지 않은 서비스는 **로그인 필요한 탭 열기** 버튼으로 바로 열 수 있습니다.
- 답변 중 멈추고 싶으면 **전체 중단**을 누르세요. 열린 AI 탭에 중단 신호를 보내고 진행 중 메시지를 실패 상태로 정리합니다.
- 입력 길이/토큰 한도 초과 문구가 감지되면 버퍼링을 멈추고 실패 메시지로 표시합니다.
- 같은 AI 채팅방을 계속 쓰는 동안에는 매번 전체 회의록을 다시 보내지 않고, 각 AI가 아직 못 본 새 발언만 추가로 전달합니다. 첫 전달 또는 커서가 없는 기존 방에서는 초기 역할/최근 맥락을 한 번 보냅니다.
- 설정에서 각 AI의 **컨셉/말투**와 **프로필 이미지 URL**을 지정할 수 있습니다. 예: "엄격하지만 챙겨주는 형처럼 말해", "귀여운 여동생처럼 밝게 정리해".
- 개인 프로필 이미지는 `extension/assets/`에 로컬로 둘 수 있으며, 이 폴더는 Git에 올리지 않습니다.
